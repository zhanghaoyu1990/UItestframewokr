...
class PySelenium(object):
    ...
    def element_wait(self, css, secs=5):
        ...
        try:
            if by == "id":
                WebDriverWait(self.driver, secs, 5).until(EC.visibility_of_element_located((By.ID, value)), messages)
            elif by == "name":
                WebDriverWait(self.driver, secs, 5).until(EC.visibility_of_element_located((By.NAME, value)), messages)
            elif by == "class":
                WebDriverWait(self.driver, secs, 5).until(EC.visibility_of_element_located((By.CLASS_NAME, value)), messages)
            elif by == "link_text":
                WebDriverWait(self.driver, secs, 5).until(EC.visibility_of_element_located((By.LINK_TEXT, value)), messages)
            elif by == "xpath":
                WebDriverWait(self.driver, secs, 5).until(EC.visibility_of_element_located((By.XPATH, value)), messages)
            elif by == "css":
                WebDriverWait(self.driver, secs, 5).until(EC.visibility_of_element_located((By.CSS_SELECTOR, value)),messages)
            # else:
                # raise NameError("Please enter the correct targeting elements,'id','name','class','link_text','xpaht','css'.")
        except Exception as e:
            logger.error("No such element found %s %s" %(str(css), e))
            return 0

    @staticmethod
    def wait_element(locator):
        """
        wait element until available
        :param locator:
        :return:
        """
        # ele = WebDriverWait(self.driver, 10).until(lambda: self.findElement(locator))
        if EC.visibility_of_element_located(locator) and EC.element_to_be_clickable(locator):
    ...
    def get_element(self, element, secs=5):
        """
        Find element
        ...
        messages = 'Element: {0} not found in {1} seconds.'.format(element, secs)
        try:
            if by == "id" or type == "ID" or type == "Id":
                # elem = WebDriverWait(self.driver, 10).until(lambda x: x.find_element_by_id(value))
                elem = WebDriverWait(self.driver, 5).until(self.wait_element((By.ID, value)), messages)
            elif by == "name" or type == "NAME" or type == "Name":
                elem = WebDriverWait(self.driver, 5).until(self.wait_element((By.NAME, value)), messages)
            elif by == "class" or type == "CLASS" or type == "Class":
                elem = WebDriverWait(self.driver, 5).until(self.wait_element((By.CLASS_NAME, value)), messages)
            elif by == "link_text" or type == "LINK_TEXT" or type == "Link_text":
                elem = WebDriverWait(self.driver, 5).until(self.wait_element((By.LINK_TEXT, value)), messages)
            elif by == "xpath" or type == "XPATH" or type == "Xpath":
                elem = WebDriverWait(self.driver, 5).until(self.wait_element((By.XPATH, value)), messages)
            elif by == "css" or type == "CSS" or type == "Css":
                elem = WebDriverWait(self.driver, 5).until(self.wait_element((By.CSS_SELECTOR, value)), messages)
            else:
                elem = None
            return elem
        except Exception as e:
            logger.error("No such element found %s %s" %(str(element), e))
            elem = None
            return elem
    ...
    def take_screen_shot(self, file_path):
        """
